...
mnist = input_data.read_data_sets('./NotMnist',one_hot=True)
...
def train_nn():
    ...
    with mg.as_default():
        ...
        with tf.Session(graph = mg) as sess:
            ...
            if not os.path.exists('./NotMnist_model/checkpoint'):
                for i in range(300):
                    epoch_cost = 0
                ...
                print(acc)
                saver.save(sess, './NotMnist_model/notmnist.ckpt')
            else:
                saver.restore(sess, './NotMnist_model/notmnist.ckpt')
                image_path = './only.png'
                ...
                return result
def main():
    if os.path.exists('./NotMnist_model/checkpoint'):
        result = train_nn()
        return result
    elif not os.path.exists('./NotMnist_model/checkpoint'):
        train_nn()
